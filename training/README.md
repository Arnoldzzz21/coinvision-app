...
This folder holds the supporting scripts and docs for the training pipeline behind the [CoinVision app](../README.md). The notebook itself, [`Codevision.ipynb`](../Codevision.ipynb), lives at the repo root next to `app.py` (the pipeline that runs the notebook needs the files here — `coin_dataset.py` and `build_new_classes.py` — but the notebook is kept at the top level for visibility).
...
| File | Purpose |
|---|---|
| [`../Codevision.ipynb`](../Codevision.ipynb) | The full pipeline: EDA, dataset build, training, fine-tuning, Grad-CAM, hierarchical experiment, evaluation. Includes executed outputs/plots from the actual run. |
...
- `cat_to_name.json` and `new_classes_manifest.csv` in the working directory (a copy of `cat_to_name.json` lives in the repo root).
